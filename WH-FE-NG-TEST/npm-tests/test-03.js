...
let countOfAgeEqual32 = 0;
let countOfAgeEqual32Array = [];
const analyseJsonData = (data) => {
  const dataArr = data.split(", key");

  const jsonCleanedData = dataArr.map((data, index) => {
    const [keyData, ageData] = data.split(", ");

    const newData = {
      key: index === 0 ? keyData.slice(4, -1) : keyData.slice(1, -1),
      age: ageData.split("=")[1],
    };
    if (newData.age === "32") {
      countOfAgeEqual32Array = [...countOfAgeEqual32Array, newData.age];
      countOfAgeEqual32 = countOfAgeEqual32 + 1;
    }
...
const writeStream = (data = []) => {
  ...
  data.forEach(function (v) {
    file.write(`key=${v.key}, age=${v.age}` + "\n");
  });
  file.end();
};

writeStream(analyseJsonData(json.data));
console.log(`THE COUNT OF ITEMS WITH AGE EQUAL TO 32 IS: ${countOfAgeEqual32}`);
